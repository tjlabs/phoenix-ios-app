...
class PhoenixBluetoothManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if let bleName = discoveredPeripheral.name {
            if bleName.contains("TJ-") {
                ...
                let validTime = (PhoenixConstants.BLE_VALID_TIME*2)
                ...
                if RSSI.intValue != 127 {
                    ...
                    if (bleScanned.contains(where: condition)) {
                        ...
                        bleScanned.updateValue(value, forKey: deviceIDString)
                    } else {
                        bleScanned.updateValue([[rssiValue, bleTime]], forKey: deviceIDString)
                    }
                    let trimmedResult = PhoenixRFDFunctions.shared.trimBLEData(bleInput: bleScanned, nowTime: bleTime, validTime: validTime)
                    switch trimmedResult {
                    case .success(let trimmedData):
                        self.bleDictionary = trimmedData
                    case .failure(let error):
                        print(getLocalTimeString() + " , (Phoenix) Error : BleManager \(error)")
                    }
    ...
    public func initBle() -> (Bool, String) {
        ...
        let message: String = localTime + " , (Phoenix) Success : Bluetooth Initialization"
        startScan(option: .Foreground)
